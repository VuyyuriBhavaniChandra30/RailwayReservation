...
<form method="POST">
    <div class="form-container">
        ...
        <div id="typeselect">
            ...
            <select name="type" value="<?php echo isset($_POST['type']) ? $_POST['type'] : ''; ?>">
...
<?php
    ...
    if (isset($_POST["GET_TRAINS"])) 
    {
        ...
        AND `to` = '" . $_SESSION["to"] . "'
        AND TIME(`sourcetime`) BETWEEN TIME('$startTime') AND TIME('$endTime')
        AND trainno NOT IN (SELECT trainno FROM cancelled WHERE date= '" . $_SESSION["date"] . "')
        AND (
            (SELECT `$day` FROM train WHERE trainno = route.trainno) = 'Y'
        )
        $sort";
        }else{
            $sql = "SELECT * FROM route 
        ...
        AND `to` = '" . $_SESSION["to"] . "'
        AND TIME(`sourcetime`) BETWEEN TIME('$startTime') AND TIME('$endTime')
        AND trainno IN (SELECT trainno FROM class WHERE classcode = '" . $_SESSION["class"] . "')
        AND trainno NOT IN (SELECT trainno FROM cancelled WHERE date= '" . $_SESSION["date"] . "')
        AND (
            (SELECT `$day` FROM train WHERE trainno = route.trainno) = 'Y'
        )
        ...
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo '</div>';
                    echo '</div>';
                    $sql3 = "SELECT * FROM stops WHERE stopname='" . $row['from'] . "' AND date='" . $_SESSION["date"] . "'";
